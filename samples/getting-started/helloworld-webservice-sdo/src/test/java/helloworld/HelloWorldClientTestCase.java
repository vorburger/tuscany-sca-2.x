...
public class HelloWorldClientTestCase {
	...
	private HelloWorldTestServerTest server;

	@Before
	public void startClient() throws Throwable {
		try {
			node = TuscanyRuntime.runComposite("helloworldwsclient.composite", "target/classes");

			helloWorldService = node.getService(HelloWorldService.class, "HelloWorldServiceComponent");

			server = new HelloWorldTestServerTest();
			server.startServer();
